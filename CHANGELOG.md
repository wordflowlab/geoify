...
1. **统一命令前缀** ✅
   - 所有命令现在都使用 `geo-` 前缀
   - 重命名文件:
     - `collect.md` → `geo-collect.md`
     - `research.md` → `geo-research.md`
     - `specify.md` → `geo-specify.md`
   - 品牌一致性更强,避免与其他工具冲突

### 使用流程改进

2. **完善"下一步"指引** ✅
   - 所有 9 个命令模板都添加了"✅ 完成!下一步"部分
   - 用户完成每一步后,明确知道接下来该做什么
   - 包含流程进度提示 (第 X/9 步)
   - 提供预计时间参考
...
4. **更新文档** ✅
   - README.md: 添加 Mermaid 流程图,更新命令列表
   - USAGE.md: 更新所有命令引用,统一使用 `geo.` 前缀
   - 添加快速导航表格
...
- ✅ 命名一致性:所有命令都有 `geo-` 前缀
- ✅ 用户体验:每一步都有明确的"下一步"指引
- ✅ 文档完整:QUICKSTART + README + USAGE 三层文档体系
- ✅ 新手友好:10 分钟快速上手
